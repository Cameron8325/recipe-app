...
class RecipeAppTests(TestCase):
    ...
    def test_search_view(self):
        self.client.login(username="testuser", password="testpassword")
        response = self.client.get(
            reverse("recipes:search_results"),
            {"query": "Test", "cooking_time": 30, "difficulty": "Easy"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_protected_views_redirect(self):
        ...
        self.assertRedirects(
            response, f"{reverse('login')}?next={reverse('recipes:recipes_list')}"
        )
        ...
        self.assertRedirects(
            response,
            f"{reverse('login')}?next={reverse('recipes:recipe_detail', args=[self.recipe.id])}",
        )
